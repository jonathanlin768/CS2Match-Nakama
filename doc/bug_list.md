...
- [x] 好友页，保存QQ/微信之后，没有外显。已增加仅绑定本人的资料读取 RPC；刷新后会把服务端私有资料安全回填到输入框，保存后同步规范化结果和资料版本。
- [x] A玩家向B玩家申请交换联系方式后，没有正确提示请求已发起，B玩家网页上没有地方收到消息，也没有地方进行交换。已增加服务端权威申请收件箱、全局待处理徽章和好友页接受/拒绝入口；DM 卡片仅作实时提示，离线或投递失败后仍可恢复申请。
- [x] 点击“接受并互相授权”时错误要求同时完善 QQ 和微信。现改为双方各自至少保存一种即可，接受时授权各自全部非空联系方式；允许 A 只有 QQ、B 只有微信等不对称组合。
