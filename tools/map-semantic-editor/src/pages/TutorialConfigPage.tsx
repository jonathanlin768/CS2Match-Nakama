...
export function TutorialConfigPage() {
  ...
  const players = useConfigStore((state) => state.documents['#Player.xlsx']?.rows ?? [])
  const teams = useConfigStore((state) => state.documents['#Team.xlsx']?.rows ?? [])
    ...
    <div className="configWorkspacePage">
      <section className="configWorkspaceMain recordEditorLayout tutorialLayout">
        <aside className="configSidebar">
          <div className="panelHeading"><div><h1>新手战斗</h1><p>{document.rows.length} 套方案 {document.dirty ? '· 未保存' : ''}</p></div></div>
        ...
        <section className="recordDetailPane">
          {!row ? <div className="emptyState large">新增或选择一套配置。</div> : <div className="detailForm">
            <header className="detailHeader"><div><h2>{rowId(row) || '未命名方案'}</h2><p>{row.enabled === true ? '当前启用' : '未启用'} · 预算 {toNumberValue(row.budget)} · {toNumberValue(row.rosterSize)} 人阵容</p></div><label className="switchField"><span>启用</span><input type="checkbox" checked={row.enabled === true} onChange={(event) => setTutorialEnabled(selectedIndex, event.target.checked)} /></label></header>
...
function PlayerMultiPicker({ title, players, selected, preferredTeam, onChange }: { title: string; players: LubanRow[]; selected: string[]; preferredTeam?: string; onChange: (value: string[]) => void }) {
  ...
  return (
    <div className="playerPicker">
      ...
      <div className="playerPickerOptions">{candidates.map((player) => { const id = rowId(player); const active = selected.includes(id); return <label key={id} className={active ? 'active' : ''}><input type="checkbox" checked={active} onChange={() => toggle(id)} /><span><strong>{displayLabel(player)}</strong><small>{toStringValue(player.teamId)} · {toStringValue(player.rarity)} · {toListValue(player.positions).join('/')}</small></span></label> })}</div>
    </div>
  )
}
