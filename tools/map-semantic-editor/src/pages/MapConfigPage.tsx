...
export function MapConfigPage() {
  ...
  function startPanelResize(kind: 'left' | 'property' | 'bottom', event: PointerEvent<HTMLDivElement>) {
    ...
    function handlePointerMove(moveEvent: globalThis.PointerEvent) {
      ...
      if (kind === 'bottom') setBottomPanelHeight(clamp(startBottom - (moveEvent.clientY - startY), 120, 420))
    }
  ...
  return (
    <main className="appShell mapConfigPage" style={{ gridTemplateRows: `auto minmax(0, 1fr) ${bottomPanelHeight}px` }}>
      <Toolbar />
      <section className="workspace" style={{ gridTemplateColumns: `${leftPanelWidth}px 6px minmax(540px, 1fr) 6px ${propertyPanelWidth}px` }}>
        <LeftPanel />
        <div className="resizeHandle verticalResizeHandle" role="separator" aria-orientation="vertical" aria-label="调整左侧面板宽度" title="调整左侧面板宽度" onPointerDown={(event) => startPanelResize('left', event)} />
        <RadarCanvas />
        <div className="resizeHandle verticalResizeHandle" role="separator" aria-orientation="vertical" aria-label="调整属性面板宽度" title="调整属性面板宽度" onPointerDown={(event) => startPanelResize('property', event)} />
        <PropertiesPanel />
      </section>
      <section className="bottomPanelFrame">
        <div className="resizeHandle horizontalResizeHandle" role="separator" aria-orientation="horizontal" aria-label="调整底部面板高度" title="调整底部面板高度" onPointerDown={(event) => startPanelResize('bottom', event)} />
        <BottomPanel />
      </section>
    </main>
  )
