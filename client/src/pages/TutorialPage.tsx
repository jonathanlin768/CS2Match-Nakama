...
export default function TutorialPage() {
  ...
  const { tables, loading, error, retry } = useGameConfig()
  ...
  if (error) return <div className="page-center"><p>教学配置暂不可用：{error}</p><div className="page-center-actions"><button className="primary-button" onClick={retry}>重新读取配置</button><button className="secondary-button" onClick={() => navigate("/")}>返回主页</button></div></div>
  if (!tables || !config) return <div className="page-center"><p>教学配置暂不可用：没有启用的方案</p><button className="secondary-button" onClick={() => navigate("/")}>返回主页</button></div>
  ...
  return <div className="sub-page tutorial-page">
    ...
    <div className="tutorial-tier-list">
      {tiers.map((tier) => <section className="tutorial-tier" key={tier.price} aria-labelledby={`tutorial-tier-${tier.price}`}>
        <header className="tutorial-tier-heading"><h2 id={`tutorial-tier-${tier.price}`}><strong>{tier.price}</strong> 元</h2><span>{tier.ids.length} 名候选</span></header>
        <div className="tutorial-tier-players">
          {tier.ids.map((id) => {
