 ...
 * 传入 hideBack 时隐藏返回箭头、不可返回（用于比赛进行中等不允许退出的界面）。
 */
export default function SubPageHeader({ title, onBack, hideBack }: SubPageHeaderProps) {
  ...
  return (
    <header className="flex h-[88px] shrink-0 items-center px-[40px]">
      {hideBack ? (
        <span className="text-3xl font-black tracking-wide text-foreground">{title}</span>
      ) : (
        <button
          type="button"
          onClick={handleBack}
