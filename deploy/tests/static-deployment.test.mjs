...
test("workflow actions are immutable and deployment uses Tailscale OIDC", () => {
  ...
  assert.match(workflow, /configure-pages-project\.mjs master/)
  assert.match(workflow, /wrangler pages deploy dist[\s\S]*--project-name "\$CLOUDFLARE_PAGES_PROJECT"[\s\S]*--branch master/)
  assert.ok(workflow.indexOf("configure-pages-project.mjs master") < workflow.indexOf("wrangler pages deploy dist"))
  assert.match(workflow, /https:\/\/\$\{FRONTEND_HOST\}\//)
  assert.match(workflow, /grep -q '<div id="root"><\/div>'/)
  assert.match(configurePages, /production_branch: target/)
  assert.match(configurePages, /method: "PATCH"/)
  assert.doesNotMatch(configurePages, /console\.(?:log|error)\([^\n]*(?:apiToken|token)/)
