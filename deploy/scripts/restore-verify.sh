...
for _ in {1..30}; do docker exec "$container" pg_isready -U verify -d verify >/dev/null 2>&1 && break; sleep 1; done
docker exec "$container" pg_isready -U verify -d verify >/dev/null 2>&1 || die "temporary PostgreSQL did not become ready"
