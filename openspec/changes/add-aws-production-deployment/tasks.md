...
- [x] 6.17 修复 Pages Direct Upload 分支错配：处理 GitHub Actions detached HEAD 被 Wrangler 推断为 `HEAD` preview 的行为，通过 Pages API 将生产分支校准为 `master`，显式按 `master` 发布并从自定义域名验证 React 入口。
