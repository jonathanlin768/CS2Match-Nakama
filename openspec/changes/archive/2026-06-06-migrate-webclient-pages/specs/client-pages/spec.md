...
MatchPage SHALL 展示模拟对战的比赛界面，包含比分栏、双方阵容、战术地图、回合事件、玩家数据和战术部署。不使用真实 WebSocket，所有数据由 stub hook 返回。
...
- **AND** 左右两侧显示 TeamRoster（T 方和 CT 方）
- **AND** 中间显示 TacticalMap（Dust2 地图）
- **AND** 底部显示 RoundEvents（暂无事件）、PlayerStats（表格）、RoundTactics（暂无战术）
